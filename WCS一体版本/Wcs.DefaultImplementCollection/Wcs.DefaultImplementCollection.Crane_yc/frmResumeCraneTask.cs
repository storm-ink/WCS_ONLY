...
namespace Wcs.DefaultImplementCollection.Crane_yc
{
    public partial class frmResumeCraneTask : Form
    {
        ...
        private void btnOk_Click(object sender, EventArgs e)
        {
            ...
            if (!验证载货台(la.State, craneDevice, la, _act, rbF1yh.Checked, tbxForkBarcode.Text.Trim(), tbxLocationBarcode.Text.Trim()))
            {
                return;
            }
        ...
        bool 验证载货台(CraneStatus status,CraneDevice craneDevice, RequestStateCommandReplyTelexTransferObject la, EquipmentAction act,Boolean 载货台是否有货,String forkBarcode, String locBarcode) 
        {
            if (载货台是否有货)
            {
                if (status != CraneStatus.有货待命)
                {
                    _logger.Warn1(string.Format("验证失败，堆垛机{0}当前状态显示为“{1}”，期望值为“{2}“。", craneDevice, status.GetDescription(), CraneStatus.有货待命.GetDescription()), this, act);
            ...
            else
            {
                if (status == CraneStatus.有货待命)
                {
                    _logger.Warn1(string.Format("验证失败，堆垛机{0}货台上当前状态显示为“{1}”。",
                        craneDevice,

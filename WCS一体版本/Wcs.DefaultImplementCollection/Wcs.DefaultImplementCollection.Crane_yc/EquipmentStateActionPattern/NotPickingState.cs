...
namespace Wcs.DefaultImplementCollection.Crane_yc
{
    ...
    public class NotPickingState : AbstractState
    {
        ...
        public override CanPerformResult CanPerform()
        {
            ...
            if (craneDevice.LastStatus.State != CraneStatus.无货待命 && craneDevice.LastStatus.State != CraneStatus.右探无货 
                                                                     && craneDevice.LastStatus.State != CraneStatus.右探有货 
                                                                     && craneDevice.LastStatus.State != CraneStatus.左探无货 
                                                                     && craneDevice.LastStatus.State != CraneStatus.左探有货 
                                                                     && craneDevice.LastStatus.State != CraneStatus.左无右无
                                                                     && craneDevice.LastStatus.State != CraneStatus.左无右有
                                                                     && craneDevice.LastStatus.State != CraneStatus.左有右无
                                                                     && craneDevice.LastStatus.State != CraneStatus.左有右有
                )
                return new CanPerformResult(false, "设备当前状态不是 无货待命 状态，不允许发送命令");
            ...
            if (currentLocation.Column != pickingLocation.Column || currentLocation.Level != pickingLocation.Level)
            {
                ...
                this.StepSerialNo = "21" + this.Context.EquipmentAction.EquipmentTaskId.ToString("000000");
                if (craneDevice.LastStatus.TaskId == this.StepSerialNo)
                    this.StepSerialNo = "211" + this.Context.EquipmentAction.EquipmentTaskId.ToString("000000");
                ...
                return new CanPerformResult(false, "设备当前不在取货点，已尝试发送行走到取货点命令");
            }
        ...
        public override void Perform()
        {
            ...
            RackLocation pickingLocation = (RackLocation)startLocation;
            RackLocation putLocation = (RackLocation)endLocation;

            //this.StepSerialNo = "QH" + this.Context.EquipmentAction.EquipmentTaskId.ToString("000000");
            this.StepSerialNo = "2" + this.Context.EquipmentAction.EquipmentTaskId.ToString("0000000");
            //if (craneDevice.LastStatus.TaskId == this.StepSerialNo)
            //    this.StepSerialNo = "22" + this.Context.EquipmentAction.EquipmentTaskId.ToString("000000");

            var barcode = this.Context.EquipmentAction.Movement.Task.ContainerCodes == null || this.Context.EquipmentAction.Movement.Task.ContainerCodes.Count() == 0 ? "" : this.Context.EquipmentAction.Movement.Task.ContainerCodes.FirstOrDefault();
            AddTaskCommand cmd = new AddTaskCommand(
                this.StepSerialNo,
                AddTaskCommandType.半自动取货,
                pickingLocation,
                putLocation,
                barcode,
                true);

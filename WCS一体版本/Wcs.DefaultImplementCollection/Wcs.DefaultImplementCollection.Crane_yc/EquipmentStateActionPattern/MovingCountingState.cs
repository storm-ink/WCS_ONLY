...
namespace Wcs.DefaultImplementCollection.Crane_yc
{
    ...
    public class MovingCountingState : AbstractState
    {
        ...
        public override IsCompeltedResult IsCompleted()
        {
            ...
            if (currentLocation.Column == pickingLocation.Column
                && currentLocation.Level == pickingLocation.Level
                && craneDevice.LastStatus.TaskId == this.Context.EquipmentAction.EquipmentTaskId.ToString()
                //&& craneDevice.LastStatus.TaskId == "PD" + this.Context.EquipmentAction.EquipmentTaskId.ToString("000000")
            ...
            return new IsCompeltedResult(false, String.Format("设备不在起点位置或者终点位置并且任务号为", this.Context.EquipmentAction.EquipmentTaskId.ToString("000000")));
        }

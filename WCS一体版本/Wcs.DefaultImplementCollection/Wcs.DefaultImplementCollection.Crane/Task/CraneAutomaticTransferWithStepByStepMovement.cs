...
{
    /// <summary>
    /// 表示堆垛机的一个单步执行的全自动的取货放货任务
    /// 到 A 点取货然后卸到 B 点
    /// </summary>
    public class CraneAutomaticTransferWithStepByStepMovement : LogicMovement
    {
        protected CraneAutomaticTransferWithStepByStepMovement()
            : base()
        {
        }
        public CraneAutomaticTransferWithStepByStepMovement(TaskableDevice device, Int32 routeId, Location startLocation, Location endLocation, Int16 containerCode)
        ...
        protected override void CreateEquipmentActions()
        {
            ...
            CraneAutomaticTransferWithStepByStepAction action = new CraneAutomaticTransferWithStepByStepAction(
                crane,
                group,
                equipmentTaskId,
                startRackLocation,

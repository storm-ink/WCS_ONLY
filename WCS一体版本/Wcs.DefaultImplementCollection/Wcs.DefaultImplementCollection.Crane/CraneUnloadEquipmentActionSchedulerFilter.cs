        ...
        public override ActionSchedulerFilterResult Filter(EquipmentActionScheduler equipmentActionScheduler, EquipmentAction action)
        {
            ...
            if (_启用急速出库)
            {
                ...
                if (_currentLocation.Synonymous.Any(x => !(x is RackLocation)))
                {
                    ...
                    if (enters.Count() != 0)
                    {
                        var _enters = enters.Select(x => x.UserCode).ToArray();
                        if (_enters.Any(x => x == action.Movement.StartLocation.UserCode))
            ...
            if (holdSignal.HandShake == HoldSignalNetTransferObjectHandShake.New)
            {
                if (_craneOutLocationIsSingle != null && _craneOutLocationIsSingle.Count() != 0 && _craneOutLocationIsSingle.Contains(conveyorLocation.DeviceCode))
                {
                    ...
                    if (__locationTask.TaskNo != 0)
                    {
                        ...
                        if (__task != null && __task.StartMotorNo == __locationTask.PosNo && _locationTask != null)

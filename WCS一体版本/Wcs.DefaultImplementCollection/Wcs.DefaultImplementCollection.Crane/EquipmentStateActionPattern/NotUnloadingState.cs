...
namespace Wcs.DefaultImplementCollection.Crane
{
    ...
    public class NotUnloadingState : AbstractState
    {
        ...
        public override CanPerformResult CanPerform()
        {
            ...
            if (craneDevice.LastStatus.DeviceState != CraneStatus.Watting || craneDevice.LastStatus.IsLoaded != 1)
        ...
        public override IsCompeltedResult IsCompleted()
        {
            ...
            if (craneDevice.LastStatus.IsLoaded == 2 && craneDevice.LastStatus.DeviceState == CraneStatus.Watting)
            {
                msg = String.Format("堆垛机单步任务完成3：{0} 放货完成，堆垛机状态：{1}", this.Context.EquipmentAction.EquipmentTaskId, craneDevice.LastStatus);
                _logger.Info(msg);
                return new IsCompeltedResult(true, msg);
            }
            else
        ...
        public override void Perform()
        {
            ...
            var equipmentTaskId = craneDevice.GetEquipmentTaskId(CraneDevice.CraneEquipmentTaskType.WcsTaskStep_Put);
            var cmd = new CraneCommand(CmdTypes.NewTask, CraneTaskTypes.StepPut, null, (RackLocation)endLocation, equipmentTaskId, (UInt32)this.Context.EquipmentAction.EquipmentTaskId, barcode);

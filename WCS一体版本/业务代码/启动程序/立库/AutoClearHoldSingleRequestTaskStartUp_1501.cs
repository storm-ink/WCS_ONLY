...
namespace ZHQXC
{
    ...
    /// 自复位占位请求处理程序：放行货物
    /// </summary>
    public class AutoClearHoldSingleRequestTaskStartUp_1501 : ThreadRunningLog, IApplicationStartup
    {
        ...
        public void check(object obj)
        {
            ...
            while (true)
            {
                ...
                try
                {
                    ...
                    //第二步：判断是否有业务占位
                    var holdSingle = conveyor.RequestBlocks.SingleOrDefault(x => x.PosNo == Convert.ToUInt16(_requestLoc.DeviceCode) && x.HandShake == RequestHandShakes.New );
                    if (holdSingle == null)
                    {

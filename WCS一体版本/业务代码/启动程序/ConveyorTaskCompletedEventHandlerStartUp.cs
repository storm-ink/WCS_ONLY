                ...
                try
                {
                    ...
                    foreach (var device in devices)
                    {
                        ...
                        foreach (var task in tasks)
                        {
                            ...
                            {
                                TaskCommand clearTaskCommand = new TaskCommand(TaskHandShakes.ApplyForDelete,task.TaskNo,"", new UInt16[0], task.RotingNo, task.From, task.To, Convert.ToUInt16(task.AtPacketIndex));
                                try
                                {

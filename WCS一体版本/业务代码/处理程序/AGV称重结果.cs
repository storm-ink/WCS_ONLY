...
namespace BOE
{
    ...
    /// 用于输送线任务报完成前读取称重结果
    /// </summary>
    public class AGV称重结果 : ITaskEventHandler<TaskCompletedEventArgs>
    {
        String[] _endLocations = new String[1] { "2006"};
        //String[] _startLocations = new String[2] { "3025", "3020" };

        Dictionary<String, String> _locations = new Dictionary<string, string>() { { "2006", "2006" } };
        ...
        public void Handle(TaskableDevice device, NHibernate.ISession session, TaskCompletedEventArgs args)
        {
            try               
            {
                if (!(device is BOE.设备相关.AGV.GSAGV.GSAGVDevice))
                {
                ...
                var action = session.Query<GSAGVSubSystemAction>().FirstOrDefault(x => x.EquipmentTaskId == args.EquipmentTaskId);
                ...
                if (!_endLocations.Contains(action.UnloadLocation.UserCode))
                {
                    String msg = String.Format("收到了 {0} 发送来的 {1} 任务完成信号，但是其起点位置 {2} 不是入货口位置，忽略本信号", device, args.EquipmentTaskId, action.LoadLocation.DeviceCode);
                    args.Handled = true;

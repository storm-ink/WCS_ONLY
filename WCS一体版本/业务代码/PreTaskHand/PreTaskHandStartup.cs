        ...
        public void Run(IWcsApplication application)
        {
            ...
            foreach (var item in PreTaskHandHelper.PreTasks)
            {
                ...
                if (item.Status == TaskStatus.Completed || item.Status == TaskStatus.Cancelled)
                    PreTaskHandHelper.PushWaitReportPreTaskList(new ReportInfo() { PreTask = item, CreatedAt = DateTime.Now, MessageType = MessageTypes.CompeletedMessage });
            }
            ...
            _reportWmsTaskStateThread = new Thread(PreTaskHandHelper.ReprotWMS);

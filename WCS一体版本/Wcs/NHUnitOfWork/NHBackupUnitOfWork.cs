...
namespace Wcs
{
    public class NHBackupServerUnitOfWork:IDisposable,INHUnitOfWork
    {
        ...
        {
            string path = System.IO.Path.Combine(System.IO.Path.GetDirectoryName(typeof(NHBackupServerUnitOfWork).Assembly.Location), "backup_server_hibernate.cfg.xml");
            ...
            else
            {
                ...
                doc.Load(filePath);
                XmlNode configurationNode=doc.SelectSingleNode("/configuration").ChildNodes.Cast<XmlNode>().Single(x=>x.Name=="backup-hibernate-configuration");

...
namespace Wcs.Framework.Cfg
{
    ...
    /// </ul>
    /// </example>
    public class ConveyorElement : DeviceElement
    {
        /// <summary>
        /// 获取位置集合节点
        /// </summary>
        public LocationsSelection LocationsSelection { get; private set; }
        /// <summary>
        /// 获取在状态更新时附加的额外动作集合节点
        /// </summary>
        ...
        /// 获取此节点映射到的输送线实例
        /// </summary>
        public NewConveyor Conveyor
        {
            get
            {
                return (NewConveyor)this.Device;
            }
        }
        /// <summary>
        /// 构造函数
        ...
        public ConveyorElement(XmlNode node)
            : base(node)
        {
        }
        ...
        public override Location CreateLocation(XmlNode locationNode)
        {
            ...
            if (locationNode.Attributes["userCode"] != null)
            {
                userCode = locationNode.Attributes["userCode"].Value;
            }

            Boolean acceptRequestSignal=false;
            if (locationNode.Attributes["acceptRequestSignal"] != null && !string.IsNullOrWhiteSpace(locationNode.Attributes["acceptRequestSignal"].Value))
            {
                acceptRequestSignal = Convert.ToBoolean(locationNode.Attributes["acceptRequestSignal"].Value);
            }

            Boolean isEntrance=false, isExit=false;
            if (locationNode.Attributes["isEntrance"] != null && !string.IsNullOrWhiteSpace(locationNode.Attributes["isEntrance"].Value))
            {
                isEntrance = Convert.ToBoolean(locationNode.Attributes["isEntrance"].Value);
            }

            if (locationNode.Attributes["isExit"] != null && !string.IsNullOrWhiteSpace(locationNode.Attributes["isExit"].Value))
            {
                isExit = Convert.ToBoolean(locationNode.Attributes["isExit"].Value);
            }

            ConveyorLocation location = new ConveyorLocation(Convert.ToInt32(locationNode.Attributes["id"].Value), userCode, acceptRequestSignal, isEntrance, isExit, this.Device);
        ...
        {
            foreach (var item in LocationsSelection.LocationElements)
            {
                item.ResolveSameAs();
            }
        }
        ...
        {
            if (node.Attributes["senderDecoder"] == null)
            {
                throw new NotFoundConfigElementAttributeException("senderDecoder", node);
            }
            if (node.Attributes["receiverDecoder"] == null)
            {
                throw new NotFoundConfigElementAttributeException("receiverDecoder", node);
            }
            string senderDecoderName = node.Attributes["senderDecoder"].Value;
            string receiverDecoderName = node.Attributes["receiverDecoder"].Value;

            NetPackageDecoder senderDecoder = Configuration.GetNetPackageDecoder(senderDecoderName);
            NetPackageDecoder receiverDecoder = Configuration.GetNetPackageDecoder(receiverDecoderName);
            NewConveyor conveyor = new NewConveyor(DeviceName, Ip, Port, ReceiveTimeout, ConnectTimeout, SendTimeout, DeviceNo, receiverDecoder as DefaultConveyorNetPackageDecoder, senderDecoder as DefaultConveyorNetPackageDecoder, LogTarget)
            {
                AllowConcurrency = AllowConcurrency
            };

            this.Device = conveyor;

            this.SequenceSelection = new SequenceSelection(node.SelectSingleNode("sequence"), this);
            this.LocationsSelection = new LocationsSelection(node.SelectSingleNode("locations"), this);
            this.UpdateStatusActionsSelection = new UpdateStatusActionsSelection(node.SelectSingleNode("updateStatusActions"));
            conveyor.UpdateStatusActions = this.UpdateStatusActionsSelection.UpdateStatusActionElements.Select(x => x.UpdateStatusAction).ToArray();
            conveyor.Locations = this.LocationsSelection.LocationElements.Select(x => x.Location as ConveyorLocation).ToArray();
        }
        ...
        {
            return this.Conveyor.Locations;
        }
    }
}

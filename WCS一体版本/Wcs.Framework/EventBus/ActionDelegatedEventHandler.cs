...
namespace Wcs.Framework.EventBus
{
        ...
        where TEvent : class,IEvent
    {
        ...
        public ActionDelegatedEventHandler(Action<TEvent> eventHandlerDelegate,Boolean hold)
        {
        ...
        {
            if (!evnt.Preservable && this.Hold)
            {
                throw new NotSupportedPreservableException<TEvent>(evnt, this);
            }
        ...
        {
            return String.Format("{0} -> {1}({2}).{3}", this.GetType(),eventHandlerDelegate.Method.ReflectedType.FullName,eventHandlerDelegate.Target, this.eventHandlerDelegate.Method.Name); 
        }
    }
}

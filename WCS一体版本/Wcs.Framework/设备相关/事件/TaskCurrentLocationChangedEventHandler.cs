...
{
    public sealed class TaskCurrentLocationChangedEventHandler : TaskEventHandler<TaskCurrentLocationChangedEventArgs>
    {
        static Logger _logger = LogManager.GetCurrentClassLogger();
        const String CounterName = "任务当前位置变化处理程序";
        public override void Handle(TaskableDevice device, TaskCurrentLocationChangedEventArgs args)
        {
                ...
                try
                {
                    ...
                    if (act != null)
                    {
                        ...
                        if (!locked)
                        {
                            ...
                            Logger.Warn1(string.Format("{0} 在处理设备任务号 {1} 时锁定失败，不再处理。", this, act), this, act);
                            unitOfWork.Commit();
                            return;
                        }
                    }

...
namespace Wcs.FrameworkExtend
{
    ...
    public static class PreTaskHandHelper
    {
        ...
        public static List<PreTask> PreTasks = new List<PreTask>();
        ...
        public static List<PreTask> PreTaskReportList = new List<PreTask>();
        public static List<PreTask> PreTaskRequestList = new List<PreTask>();
            ...
            lock (PreTasks)
            {
                ...
                if (pretask.Source == TaskSource.Wms)
                {
                    if (pretask.Status == TaskStatus.Executing)
                        PushWaitReportPreTaskList(pretask);
                }
                ...
                if (pretask.Status == TaskStatus.Cancelled || pretask.Status == TaskStatus.Completed)
                {
                    if (pretask.Source == TaskSource.Wms)
                        ...
                        PushWaitRequestPreTaskList(pretask);
                }
            }
        }
        ...
        #endregion
        #region 待上报
        public static bool PushWaitReportPreTaskList(PreTask preTask)
        {
            lock (PreTaskReportList)
            {
                if (!PreTaskReportList.Any(x => x.TaskCode == preTask.TaskCode))
                    PreTaskReportList.Add(preTask);
                return true;
            }
        }

        public static void PopWaitReportPreTaskList(PreTask preTask)
        {
            lock (PreTaskReportList)
            {
                var ri = PreTaskReportList.FirstOrDefault(x => x.TaskCode == preTask.TaskCode);
                if (ri != null)
        ...
        #endregion
        #region 待请求
        public static bool PushWaitRequestPreTaskList(PreTask preTask)
        {
            lock (PreTaskRequestList)
            {
                if (!PreTaskRequestList.Any(x => x.TaskCode == preTask.TaskCode))
                    PreTaskRequestList.Add(preTask);
                return true;
            }
        }

        public static void PopWaitRequestPreTaskList(PreTask preTask)
        {
            lock (PreTaskRequestList)
            {
                var ri = PreTaskRequestList.FirstOrDefault(x => x.TaskCode == preTask.TaskCode);
                if (ri != null)
                    PreTaskRequestList.Remove(ri);
            }
        }
        #endregion
        ...
        private static void CompeletedHand()
        {
            ...
            foreach (var item in taskCloned)
            {
                ...
                using (NHUnitOfWork unitOfWork = new NHUnitOfWork())
                {
                    ...
                    if (pretask != null && pretask.Status != item.Status)
                    {
        ...
        #endregion
        #region 上报上位系统
        public static void PreTaskReportUpHandProc()
        {
            while (true)
            {
                try
                {
                    ...
                    PreTask[] reportInfosCloned;
                    lock (PreTaskReportList)
                    {
                        reportInfosCloned = PreTaskReportList.ToArray();
                    }

                    for (int i = 0; i < reportInfosCloned.Length; i++)
                    {
                        try
                        {
                            var item = reportInfosCloned[i];
                            if (ExistArchiveProcPreTaskList(item))
                            {
                                PopWaitRequestPreTaskList(item);
                                continue;
                            }

                            var handlers = Wcs.FrameworkExtend.Cfg.WcsConfiguration.Instance.WMSTaskCompeletedExternalHandlersElement.WMSPreTaskCompeletedExternalHandlers.Where(x => x.Allowed(item));
                            if (handlers.Count() > 0)
                            {
                                foreach (var hand in handlers)
                                {
                                    if (hand.Allowed(item))
                                        hand.Hand(item);
                                }
                            }
                            else
        ...
        #endregion
        #region WCS任务完成请求处理
        public static void PreTaskRequestHandProc()
        {
            while (true)
            {
                try
                {
                    ...
                    PreTask[] preTaskListCloned;
                    lock (PreTaskRequestList)
                    {
                        preTaskListCloned = PreTaskRequestList.ToArray();
                    }

                    for (int i = 0; i < preTaskListCloned.Length; i++)
                    {
                        try
                        {
                            var item = preTaskListCloned[i];
                            if (ExistArchiveProcPreTaskList(item))
                            {
                                PopWaitRequestPreTaskList(item);
                                continue;
                            ...
                            var handlers = Wcs.FrameworkExtend.Cfg.WcsConfiguration.Instance.TaskRequestHandlersElement.WCSPreTaskRequestHandlers.Where(x => x.Allowed(item));
                            if (handlers.Count() > 0)
                            {
                                foreach (var hand in handlers)
                                {
        ...
        public static void PushDeletePreTaskList(PreTask PreTask)
        {
            lock (_waitDeletePreTasks)
            {
                ...
                {
                    _logger.Warn1(string.Format("{0}已存在于预任务归档队列当中", PreTask), typeof(PreTaskHandHelper), PreTask);
                    return;
                }

                _waitDeletePreTasks.Add(PreTask);

                _logger.Debug1(string.Format("{0}被加入预任务归档队列", PreTask), typeof(PreTaskHandHelper), PreTask);
            }
        }
        ...
        public static void PopDeletePreTaskList(PreTask PreTask)
        {
            lock (_waitDeletePreTasks)
            {
                ...
                if (item == null)
                {
                    _logger.Warn1(string.Format("{0}已被从归档队列移除", item), typeof(PreTaskHandHelper), item);
                    return;
                }

                _waitDeletePreTasks.Remove(item);

                _logger.Debug1(string.Format("{0}被移出归档队列", item), typeof(PreTaskHandHelper), item);
            }
        }
            ...
            while (true)
            {
                ...
                try
                {
                    ...
                    foreach (var item in PreTasksCloned)
                    {
                        try
                        {
                            _logger.Trace1(string.Format("准备将{0}预任务{1}归档...", item.TaskType, item), typeof(PreTaskHandHelper), item);
                            ...
                            _logger.Debug1(string.Format("{0} 归档成功.", item), typeof(PreTaskHandHelper), item);
                        ...
                        {
                            _logger.Error1(ex, typeof(PreTaskHandHelper), item);
                        }
                    }
                }
                catch (Exception ex)
